...
1. Download the .xls file from the website using Chrome

2. Open the file in Excel and navigate to `Fig 3b`

3. Copy the data in `Fig 3b`, and paste it into a new excel file which I named `Comp Stat Lab 5 Data`

4. Save `Comp Stat Lab 5 Data` as a `.csv` file

5. Delete the middle two columns, which are empty

6. Delete the first row of headers, which are unnecessary

7. Notice that the headers of the sample columns were the same, as were the virus columns. We can append the values in the second column of each data category in order to create a dataframe that is 2 columns in width.

8. Rename the headers "sample" and "viruses"

9. Read in the `.csv` file using:

```{r}
dat.kitsberg = read.csv("/Users/hudson/Downloads/Comp Stat Lab 5 Data.csv")
```

9. Ensure that the data looks correct using: 
...
dat.kitsberg.cleaned = filter(
  ...
  dat.kitsberg.cleaned$Treatment != "uninfected"
)
head(dat.kitsberg.cleaned)
```
...
dat.kitsberg.cleaned = dat.kitsberg.cleaned |>
  ...
  mutate(Treatment = case_when(Treatment == "12hpi" ~ "THP-1"))
head(dat.kitsberg.cleaned)
```
...
```{r}
#| label: fig-HCMV-by-Cell-Type
#| fig-cap: "Quantification of viral genomes detected in the nuclei of infected primary or THP1 monocytes (n = 87 and n = 112, respectively) and macrophages (n = 93 and n = 109, respectively) by DNA-FISH at 12 hpi. The P-value was calculated using Poisson regression. Source data are provided as a Source data file. "
#| fig-width: 5
#| fig-height: 3
#| scale: "80%"
#| fig-align: "center"
...
```{r}
#| label: tbl-summary-kitsberg
#| tbl-cap: "Numerical Summary of Kitsberg Data"
#| tbl-align: "center"
#| size: scriptsize
group_by(dat.kitsberg.cleaned, SampleType, CellType) |>

    summarize(
        n = n(),
        Mean = mean(Viruses),
        SD = sd(Viruses),
        Median = median(Viruses),
        IQR = IQR(Viruses),
        Skewness = e1071::skewness(Viruses),
        "Excess Kurtosis" = e1071::kurtosis(Viruses)
    )|>
...
dat.bal.cleaned = dat.bal |> mutate(price = (10^SPLog)/1000000) |>
  filter(!is.na(SPLog))
# I noticed there were a lot of completely empty rows at the bottom, so I removed them.
```
...
```{r}
dat.bal.cleaned = dat.bal.cleaned |> mutate(OriginCountry = case_when(
  Japan == 1 ~ "Japan",
...
```{r}
dat.bal.cleaned = dat.bal.cleaned |> mutate(Manufacturer = case_when(
...
ggplot(dat.bal.cleaned) + 
  geom_violin(aes(x = Manufacturer,
                  y = price),
  ...
  geom_boxplot(aes(x = Manufacturer,
                  ...
                  width = 0.1) + 
  labs(x = "Manufacturer",
       y = "Price in Millions of Dollars")+
  ggtitle("Price vs Manufacturer") 
```

```{r}
summary = dat.bal.cleaned |> 
...
```{r}
ggplot(dat.bal.cleaned) + 
  geom_point(aes(x = Engines.RPM,
                  y = price)) +
  geom_smooth(aes(x = Engines.RPM,
                  y = price),
                  method = "lm",
                  color = "black") + 
  labs(x = "RPM of Engines",
       y = "Price in Millions of Dollars")+
  ggtitle("Price vs Engine RPM") 
```


```{r}
summary.2 = dat.bal.cleaned |> filter(!is.na(Engines.RPM)) |>
summarize(
Pearson = cor(Engines.RPM, price, method="pearson"),
Spearman = cor(Engines.RPM, price, method="spearman"),
Kendall = cor(Engines.RPM, price, method="kendall")
)
  knitr::kable(summary.2, digits = 2)
```
